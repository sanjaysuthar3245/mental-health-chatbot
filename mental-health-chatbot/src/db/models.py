...
class MoodEntry(db.Model):
    ...
    physical_activity = db.Column(db.Integer, nullable=True)  # 1-10 scale
    social_activity = db.Column(db.Integer, nullable=True)  # 1-10 scale
...
class UserAchievement(db.Model):
    ...
    achievement_metadata = db.Column(db.Text, nullable=True)  # JSON string for additional data
    earned_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))
    
    def get_metadata(self):
        """Get achievement metadata as dictionary"""
        if self.achievement_metadata:
