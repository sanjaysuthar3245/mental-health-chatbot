...
class MentalHealthClassifier:
    ...
    def predict_mental_health_status(self, 
                                   ...
                                   context_features: Dict[str, Any] = None) -> Dict[str, Any]:
        """Predict mental health status from features with enhanced context awareness"""
        ...
        try:
            ...
            # Process numerical features with enhanced set
            if numerical_features and self.scaler:
                numerical_array = np.array([[
                    ...
                    numerical_features.get('appetite_score', 5),
                    numerical_features.get('concentration_score', 5),
                    numerical_features.get('motivation_score', 5),
                    numerical_features.get('anxiety_level', 5)
                ]])
                numerical_scaled = self.scaler.transform(numerical_array)
            else:
                numerical_scaled = np.zeros((1, 10))
            
            # Process context features
            context_vector = self._process_context_features(context_features) if context_features else np.zeros((1, 5))
            
            # Combine all features
            combined_features = np.hstack([text_vector.toarray(), numerical_scaled, context_vector])
            ...
            risk_level = self._assess_enhanced_risk_level(predicted_class, max(probabilities), context_features)
            
            # Generate personalized recommendations
            recommendations = self._get_enhanced_recommendations(predicted_class, context_features, numerical_features)
            ...
            return {
                ...
                'severity_indicators': self._extract_severity_indicators(predicted_class, numerical_features),
                'next_steps': self._get_next_steps(predicted_class, risk_level),
                'monitoring_advice': self._get_monitoring_advice(predicted_class)
            }
    ...
    def _process_context_features(self, context_features: Dict[str, Any]) -> np.ndarray:
        """Process context features for enhanced prediction"""
        if not context_features:
            return np.zeros((1, 5))
        
        # Extract context features
        time_of_day = context_features.get('time_of_day', 'morning')
        day_of_week = context_features.get('day_of_week', 'weekday')
        recent_events = context_features.get('recent_events', [])
        social_context = context_features.get('social_context', 'alone')
        weather = context_features.get('weather', 'neutral')
        
        # Encode time of day (0-3)
        time_encoding = {'morning': 0, 'afternoon': 1, 'evening': 2, 'night': 3}.get(time_of_day, 0)
        
        # Encode day of week (0-1)
        day_encoding = 0 if day_of_week == 'weekday' else 1
        
        # Count recent stressful events
        stressful_events = sum(1 for event in recent_events if event.get('stress_level', 0) > 5)
        event_encoding = min(stressful_events / 3, 1)  # Normalize to 0-1
        
        # Encode social context (0-2)
        social_encoding = {'alone': 0, 'with_friends': 1, 'with_family': 2}.get(social_context, 0)
        
        # Encode weather (0-2)
        weather_encoding = {'sunny': 0, 'neutral': 1, 'rainy': 2}.get(weather, 1)
        
        return np.array([[time_encoding, day_encoding, event_encoding, social_encoding, weather_encoding]])
    
    def _assess_enhanced_risk_level(self, predicted_class: str, confidence: float, context_features: Dict[str, Any] = None) -> str:
        """Enhanced risk assessment considering context and confidence"""
        base_risk = self._assess_risk_level(predicted_class, confidence)
        ...
        if context_features:
            recent_events = context_features.get('recent_events', [])
            stressful_events = sum(1 for event in recent_events if event.get('stress_level', 0) > 7)
            
            if stressful_events > 2:
                return 'high' if base_risk == 'medium' else 'medium'
        
        return base_risk
    
    def _get_enhanced_recommendations(self, predicted_class: str, context_features: Dict[str, Any] = None, numerical_features: Dict[str, float] = None) -> List[str]:
        """Get enhanced, personalized recommendations"""
        base_recommendations = self._get_class_recommendations(predicted_class)
        
        # Add context-specific recommendations
        if context_features:
            time_of_day = context_features.get('time_of_day', 'morning')
            if time_of_day == 'evening' and 'sleep' not in ' '.join(base_recommendations).lower():
                base_recommendations.append('Establish a calming bedtime routine')
        
        # Add numerical feature-based recommendations
        if numerical_features:
            sleep_hours = numerical_features.get('sleep_hours', 8)
            if sleep_hours < 6:
                base_recommendations.append('Prioritize getting 7-9 hours of sleep nightly')
            
            physical_activity = numerical_features.get('physical_activity', 5)
            if physical_activity < 4:
                base_recommendations.append('Consider incorporating regular physical activity')
        
        return base_recommendations
    
    def _extract_severity_indicators(self, predicted_class: str, numerical_features: Dict[str, float] = None) -> Dict[str, Any]:
        """Extract severity indicators from prediction and features"""
        indicators = {
        ...
        if 'severe' in predicted_class:
            ...
            indicators['key_concerns'].append('High severity mental health symptoms')
        elif 'moderate' in predicted_class:
            indicators['overall_severity'] = 'moderate'
            indicators['key_concerns'].append('Moderate mental health symptoms')
        
        if numerical_features:
            if numerical_features.get('sleep_hours', 8) < 6:
                indicators['key_concerns'].append('Insufficient sleep')
                indicators['improvement_areas'].append('Sleep hygiene')
            
            if numerical_features.get('energy_level', 5) < 4:
                indicators['key_concerns'].append('Low energy levels')
                indicators['improvement_areas'].append('Energy management')
            
            if numerical_features.get('social_activity', 5) < 4:
                indicators['key_concerns'].append('Limited social engagement')
                indicators['improvement_areas'].append('Social connection')
        
        return indicators
    
    def _get_next_steps(self, predicted_class: str, risk_level: str) -> List[str]:
        """Get specific next steps based on prediction and risk level"""
        if risk_level == 'high':
            return [
            ...
            return [
                ...
                'Start implementing recommended coping strategies',
    ...
    def _get_monitoring_advice(self, predicted_class: str) -> Dict[str, Any]:
        """Get monitoring advice for the predicted class"""
        monitoring_advice = {
            'frequency': 'daily',
            'key_metrics': ['mood', 'sleep', 'energy'],
            'warning_signs': [],
            'tracking_tools': []
        }
        
        if 'depression' in predicted_class:
            monitoring_advice['key_metrics'].extend(['appetite', 'motivation', 'social_engagement'])
            monitoring_advice['warning_signs'].extend(['persistent sadness', 'loss of interest', 'hopelessness'])
            monitoring_advice['tracking_tools'].append('PHQ-9 questionnaire')
        
        if 'anxiety' in predicted_class:
            monitoring_advice['key_metrics'].extend(['anxiety_level', 'worry_frequency', 'physical_symptoms'])
            monitoring_advice['warning_signs'].extend(['excessive worry', 'panic attacks', 'avoidance'])
            monitoring_advice['tracking_tools'].append('GAD-7 questionnaire')
        
        if 'severe' in predicted_class:
            monitoring_advice['frequency'] = 'multiple times daily'
            monitoring_advice['warning_signs'].extend(['crisis thoughts', 'self-harm ideation'])
